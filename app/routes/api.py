...
async def make_trade(portfolio_id: int, trade: TradeSummary, db: AsyncSession = Depends(get_db)):
    t = Trade(
        portfolio_id=portfolio_id,
        ticker=trade.ticker,
        side=trade.side,
    ...
    return TradeOut(
        id=t.id, ticker=t.ticker, portfolio_id=t.portfolio_id,
        side=t.side, amount=t.amount, price=t.price,
        trade_time=str(t.trade_time), status=t.status
    )
